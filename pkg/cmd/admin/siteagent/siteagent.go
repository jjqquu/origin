...
you have failover protection.`
	...
	siteagentExample = `  # See what the siteagent would look like if created
...
func NewCmdSiteAgent(f *clientcmd.Factory, parentName, name string, out io.Writer) *cobra.Command {
	...
	cmd.Flags().Int32Var(&cfg.Replicas, "replicas", cfg.Replicas, "The replication factor of the siteagent; commonly 2 when high availability is desired.")
...
func RunCmdSiteAgent(f *clientcmd.Factory, cmd *cobra.Command, out io.Writer, cfg *SiteAgentConfig, args []string) error {
	...
	if errs := cfg.Action.WithMessage(fmt.Sprintf("Creating site agent %s", cfg.Name), "created").Run(list, namespace); len(errs) > 0 {
		return errExit
	}
	return nil
}
